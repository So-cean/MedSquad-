...
每轮对话由 LLM 实时决定下一步谁行动、说什么、是否移动——真正的"多智能体实时对话"。
...
一次性生成全部 ActionPlan，然后转成 Timeline 一次性播放。
...
| 特性 | 逐轮交互模式 | 预生成模式 |
|------|-------------|-----------|
| CLI 入口 | `interactive_demo.py` | `terminal_demo.py` |
| 对话生成 | 每轮调用 LLM 决定下一步 | 一次性生成全部步骤 |
| 终端输出 | 逐轮实时打印 | 一次性全部打印 |
| 对话质量 | LLM 可根据上下文动态调整 | 全部预生成，灵活性低 |
| 速度 | 每轮等 API 响应 | 一次 API 调用 |
| 支持 mock | 是（`--mock`） | 是（`EDMAS_AGENT_PROVIDER=mock`） |
| 适用场景 | 演示、调试、追求真实感 | 快速批量生成 |
...
│   │   ├── mock_agent.py     # 确定性 Mock 智能体（预生成模式，不依赖 API）
│   │   ├── openai_agent.py   # OpenAI API 智能体（预生成模式，一次性生成全部计划）
│   │   ├── step_agent.py     # 逐轮对话智能体（每轮调用 LLM 决定下一步）★新增
│   │   │   ├── StepAgent     # API 驱动，每轮 next_step()
│   │   │   └── MockStepAgent # 确定性本地脚本，离线可用
...
│   │
│   ├── cli/
│   │   ├── terminal_demo.py      # 预生成模式入口（一次性生成全部计划）
│   │   └── interactive_demo.py   # 逐轮交互模式入口（每轮 LLM 决定下一步）★新增
│   │
│   └── data/examples/        # 预置案例
...
程序会提示输入场景，然后**逐轮**进行对话——每轮 LLM 决定谁说话、说什么、是否移动，终端实时打印。
...
# 使用预置案例
python -m app.cli.interactive_demo --case stomach
python -m app.cli.interactive_demo --case headache

# 使用 mock 模式（不依赖 API，快速演示）
python -m app.cli.interactive_demo --intent "车祸急诊流程" --mock

# 保存生成的 SimulationSpec JSON
python -m app.cli.interactive_demo --intent "胃痛病人看病" --save-generated-spec runtime/my_case.json
```
...
**逐轮交互模式（interactive_demo.py）：**

```
SimulationSpec → WorldState → 主循环:
  while True:
    StepAgent.next_step(spec, world_state, history)  ← 每轮调用 LLM
    终端打印本轮动作
    更新 world_state（位置、状态）
    追加到 history
    if end_simulation: break
→ 保存 Timeline
```

**预生成模式（terminal_demo.py）：**

```
SimulationSpec → WorldState → 一次性 generate_plan() → Timeline → 打印
```
...
- **`MockAgent`**（`app/agents/mock_agent.py`）：确定性本地实现。检测是否为创伤案例，一次性生成全部对话和设备使用流程。适合开发调试和离线演示。
- **`OpenAIAgent`**（`app/agents/openai_agent.py`）：API 驱动。将 `SimulationSpec` + `WorldState` + 每个角色的 system prompt 发给 LLM，一次性生成完整 `ActionPlan`。

### 7.4 逐轮对话智能体（逐轮交互模式）★新增

- **`StepAgent`**（`app/agents/step_agent.py`）：每轮调用 LLM 决定**一个**下一步动作。输入包含 `SimulationSpec` + 当前 `WorldState` + 对话历史 + 角色职责，输出一个 `PlanStep`。LLM 可以动态调整对话策略——比如根据之前的对话决定谁发言、是否提前结束。
- **`MockStepAgent`**：确定性本地脚本，根据场景类型（创伤/普通）生成预定义的步骤序列，离线可用。通过 `--mock` 参数启用。

**StepAgent 的 system prompt 包含：**
- 精确的 actor_id 列表（防止 LLM 使用 display_name 而非 id）
- 精确的 location_id 和 device_id 列表
- 每个角色的职责描述
- 地图上下文
- 对话历史摘要

**StepAgent 的容错机制：**
- LLM 返回 null 字段时自动填充默认值
- LLM 使用 display_name 而非 id 时自动纠正
- API 调用失败时自动回退到 `MockStepAgent`

### 7.5 校验服务（`app/services/validation_service.py`）
...
### 7.6 地图上下文服务（`app/services/map_context_service.py`）
